...
if ($request->image) {
    ...
    $fileName = Str::slug($request->name) . '-' . date('YmdHis') . '.' . $extension;
    ...
    // save image
    Image::make($request->image)->save($folder . '/' . $fileName);

    // save path in DB
    $testimonial->image = 'uploads/custom-images/' . $fileName;
}
    ...
    // Save image using Intervention Image to the custom folder
    $full_path = $folder . '/' . $fileName;
    Image::make($request->image)->save($full_path);

    $testimonial->image = 'uploads/custom-images/' . $fileName;
    $testimonial->save();

    // Delete old file if exists
    if($existing_file && File::exists(base_path($existing_file))){
        File::delete(base_path($existing_file));
    }
}
