...
if ($request->logo) {
    ...
    // filename only
    $fileName = 'logo-' . date('Y-m-d-H-i-s-') . rand(1000, 9999) . '.' . $ext;

    // ABSOLUTE SERVER PATH
    $folder = '/home/u807348773/domains/seocompanygurugram.com/public_html/travelntime/uploads/custom-images';
    ...
    $logo_setting->value = 'uploads/custom-images/' . $fileName;
    $logo_setting->save();

    // delete old logo
    if ($old_logo) {
        $oldPath = '/home/u807348773/domains/seocompanygurugram.com/public_html/travelntime/' . $old_logo;
        ...
        if ($request->footer_logo) {
            ...
            // filename only
            $fileName = 'footer-logo-' . date('Y-m-d-H-i-s-') . rand(1000, 9999) . '.' . $ext;
            ...
            // save RELATIVE path in DB
            $footer_logo_setting->value = 'uploads/custom-images/' . $fileName;
        ...
        if ($request->hasFile('secondary_logo')) {
            ...
            $fileName = 'secondary-logo-' . date('Y-m-d-H-i-s-') . rand(1000, 9999) . '.' . $ext;
        
            // ABSOLUTE SERVER PATH (as you want)
            $folder = '/home/u807348773/domains/seocompanygurugram.com/public_html/travelntime/uploads/custom-images';
            ...
            // move image to absolute path
            $image->move($folder, $fileName);
        
            // save RELATIVE path in DB (important)
            $secondary_logo_setting->value = 'uploads/custom-images/' . $fileName;
        ...
        if ($request->favicon) {
            ...
            $fileName = 'favicon-' . date('Y-m-d-H-i-s-') . rand(1000, 9999) . '.' . $ext;
            ...
            // delete old favicon
            if ($old_favicon) {
                $oldPath = '/home/u807348773/domains/seocompanygurugram.com/public_html/travelntime/' . $old_favicon;
                if (File::exists($oldPath)) {
                    unlink($oldPath);
                }
            }
