...
{
    $query = Destination::with('translation')->withCount('services');

    if (str_contains($request->path(), 'domestic')) {
        $query->where('country', 'india');
    } elseif (str_contains($request->path(), 'international')) {
        $query->where('country', '!=', 'india')->whereNotNull('country');
    }
