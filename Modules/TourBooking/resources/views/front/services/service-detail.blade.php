        ...
        <div class="col-12">
            <div class="tg-tour-details-video-thumb p-relative mb-15">
                @if (isset($nonThumbnails[0]))
                    <a href="{{ asset('storage/' . $nonThumbnails[0]->file_path) }}"
                       class="glightbox"
                       data-gallery="service-gallery">

                        <img class="w-100"
                             src="{{ asset('storage/' . $nonThumbnails[0]->file_path) }}"
...
@for ($i = 1; $i <= 2; $i++)
    @if (isset($nonThumbnails[$i]))
        <div class="col-lg-6 col-md-6">
            <div class="tg-tour-details-video-thumb mb-15 position-relative">

                <a href="{{ asset('storage/' . $nonThumbnails[$i]->file_path) }}"
                   class="glightbox"
                   data-gallery="service-gallery">
...
@endforeach
                    ...
                    <div class="row align-items-center">
                        ...
                        <div class="col-lg-4">
                            <div class="tg-tour-details-video-feature-price mb-15">
                               <button type="button" class="tg-btn tg-btn-switch-animation w-100" onclick="openPopup()">Book now</button>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="tg-tour-about-wrap mr-55">
                            <div class="tg-tour-about-content">
                                <div class="tg-tour-about-inner mb-25">
                                    @if(strtolower($service?->serviceType?->name) !== 'visa')
                    ...
                    <div class="col-xl-3 col-lg-4">
                        <div x-data="bookingForm()" class="tg-tour-about-sidebar top-sticky mb-50"> 
                            ...
                            <form action="{{ route('front.tourbooking.book.checkout.view') }}">
                                @if(strtolower($service?->serviceType?->name) === 'visa')
                                    <h4 class="tg-tour-about-title title-2 mb-15">Book This Visa</h4>
                                @else
                                ...
                                <div class="tg-tour-about-coast d-flex align-items-center flex-wrap justify-content-between mb-20">
                                    @if ($service?->serviceType?->name)
                                    <span class="tg-tour-about-sidebar-title d-inline-block"> {{ __('translate.Type') }} </span>
                                    <h5 class="total-price"  >{{ $service?->serviceType?->name }}</h5>
                                    @endif
                                </div>

                                <div class="tg-tour-about-coast d-flex align-items-center flex-wrap justify-content-between mb-20">
                                    @if ($service?->group_size)
                                    <span class="tg-tour-about-sidebar-title d-inline-block"> {{ __('translate.Group Size') }} </span>
                                    <h5 class="total-price"  >{{ $service?->group_size }}</h5>
                                    @endif
                                </div>

                                <div class="tg-tour-about-coast d-flex align-items-center flex-wrap justify-content-between mb-20">
                                    @if ($service?->duration)
                                    <span class="tg-tour-about-sidebar-title d-inline-block"> {{ __('translate.Duration') }} </span>
                                    <h5 class="total-price"  >{{ $service?->duration }}</h5>
                                    @endif
                                </div>
  ...
  <div class="popup-content">
    ...
    <h2>Booking Form</h2>
<h4 class="text-lg font-bold mb-4">Contact Us to Book This Tour</h4>
        <form method="POST" action="{{ route('front.tourbooking.enquiry.store') }}">
            @csrf
            <input type="hidden" name="service_id" value="{{ $service->id }}">
            ...
            <div class="flex justify-between">
                <button type="button" onclick="closePopup()" class="tg-btn tg-btn-switch-animation w-40">Cancel</button>
                <button type="submit" class="tg-btn tg-btn-switch-animation w-40">Send Enquiry</button>
            </div>
        </form>
  </div>
</div>
