...
class FrontEndManagementController extends Controller
{
    ...
    {
        // Fetch existing images from DB (assuming a table 'working_process' with one row)
        $data = DB::table('working_process')->first();
        return view('admin.frontend-management.working_process', compact('data'));
    }
    
    public function workingProcessStore(Request $request)
    {
        $request->validate([
            ...
            'image_4' => 'nullable|image|mimes:jpg,jpeg,png,webp',
        ]);
    ...
    for ($i = 1; $i <= 4; $i++) {
        if ($request->hasFile('image_'.$i)) {
            ...
            $file->move(public_path('uploads/working_process'), $filename);
            $update['image_'.$i] = 'uploads/working_process/'.$filename;
        }
    }

    if ($data) {
        DB::table('working_process')->update($update);
    } else {
        DB::table('working_process')->insert($update);
    }

    return back()->with('success', 'Working Process images updated successfully!');
}
